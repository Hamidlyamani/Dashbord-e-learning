...
export function ContainerChat(user: user) {
    ...
    useEffect(() => {
        const currentUserMessages = messages_of_users.find(item => item.user === user.id);
        setUserMessages(currentUserMessages || null); // Set to null if no messages found for the user
    }, [user]); 
            ...
            <div className="w-3/4 bg-white shadow-xl relative rounded-[15px] max-h-full overflow-auto ">
                <div className="flex justify-between py-2 px-8 items-start border-b border-[#DFDFDF] w-full ">
                    <div className="flex gap-2">
                        <img src={user.avatar} alt='' className=" object-cover w-[56px] h-[56px] rounded-full" />
                        <div >
                            <div className="flex gap-2 items-center"><p className="font-Urb_medium text-[20px] text-[##515151]">{user.name}</p> <p className="text-blue text-[14px] ">online</p></div>
