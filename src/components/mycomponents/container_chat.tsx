...
export function ContainerChat(data: user) {
    ...
    }, [userMessages]); // scrolls every time messages update

    return (
        <>
            <div className="w-3/4 bg-white shadow-xl relative rounded-[15px] max-h-full flex flex-col overflow-hidden  h-[calc(100vh-150px)]">
                ...
                <div className="containerMessages py-2 px-8 relative h-full overflow-y-scroll pb-20 "
                    ref={containerRef} >
                    {userMessages?.messages?.map((message, index) => (
                        <Message key={index} {...message} />
                    ))}
                </div>
                <div className="flex justify-end p-4 items-center border-t bottom-0 w-full absolute bg-white ">
                    ...
                    <button className="bg-blue text-white px-4 py-2 flex items-center gap-2 rounded-md">Envoyer
                        <span className="min-w-6"><img src={sand_button} alt="" /></span></button>
                </div>
            </div >
        </>
    )
